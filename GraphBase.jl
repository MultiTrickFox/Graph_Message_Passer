...
update_node_wrt_neighbors!(node) =
    ...
    if any([edge.encoding != nothing && edge.node_to.encoding != nothing for edge in node.edges])
        ...
        attentions = softmax(vcat([prop(node.nn[1:1], edge.encoding; act2=nothing) for edge in node.edges if edge.encoding != nothing && edge.node_to.encoding != nothing]...))
        attended = sum(incomings .* attentions, dims=1)
        node.collected = prop(node.nn[2:2], attended)
    end
